...
router.get('/', async (req, res) => {
    try {
        const keys = await queries.getAllKeys.all();
...
router.post('/generate', async (req, res) => {
    try {
        ...
        for (let i = 0; i < Math.min(count, 50); i++) {
            ...
            await queries.createKey.run(keyValue, plan_name, duration_days, username, mobile);
...
router.put('/:id/revoke', async (req, res) => {
    try {
        await queries.revokeKey.run(req.params.id);
